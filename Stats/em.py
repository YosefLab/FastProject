...
def em_exp_norm_mixture2(zmat, cutoff):
    """This version is different in that it ignores locations where
    expression is equal to zero when adjusting distributions"""    
    ...
    nz_zmat = sum(zmat != 0, axis=1);
    
    gamma[zmat==0] = 0; 
    gamma = gamma.astype(float64);
    Pi = (sum(gamma, 1) / nz_zmat).reshape((zmat.shape[0],1)); #average ignoring zeros
    ...
    mu_l=weighted_mean(zmat,gamma_n);
    mu_h=weighted_mean(zmat,gamma);
    st_h=weighted_std(zmat,gamma);
    st_l=weighted_std(zmat,gamma_n);
    st_l[isnan(st_l)]=1e-3;
    ...
    for niter in arange(0,150)+1:
        ...
        gamma = (Pix * p_high) / ( ((1-Pix)*p_low) + (Pix*p_high));
        gamma[zmat==0] = 0;  #weighted averages will ignore zero location        
    ...
    min_g = min(gamma[gamma!=0]);  #fix the zero values at the minimum
    gamma[gamma==0] = min_g;

    #Since this ignores zeros, if zmat has a row with all zeros, we get NaNs
    #Also, Nans may show up for low number of samples
